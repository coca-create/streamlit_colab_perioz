...
def expire():
    ...
    expiration_date = datetime(2024, 12, 31)  # 期限日を設定
    pathname=os.path.join(data_folder,'settings2.json')
    if datetime.now() > expiration_date:
        if not os.path.exists(pathname):
            with open(pathname,'w',encoding='utf-8')as f:
                f.write("keywords")
        print("このプログラムの有効期限は2024年12月31日です。有効期限が切れました。")
...
def main():
    ...
    tab_labels = ["配布字幕の再編", "翻訳お手伝い", "Word/Excel↔SRT/VTT/TXT", "SRT/VTT→Excel","whisperファイルの復活","whisperファイルの復活②","YT付属字幕の再編","日本語srt,vttの句読点分割","単語置き換え管理","Dot管理","その他","LICENSE"]
        # サイドバーにタブの選択肢を表示
    ...
    if st.session_state.current_tab=="単語置き換え管理": #☆単語
        ...
        with st.container():
            ...
            with left_column:
                ...
                replacement_word = st.text_input("置き換え後の単語",key="rp_input2")
        ...
        with st.container():
            #load_multi_without_dot()
    
            #print(f"[pre]dot_note:{st.session_state.select_dot_note}")
            #print(f"[pre]dot_key:{st.session_state.dot_key}")
            #print(f"[pre]dot_result:{st.session_state.select_dot_result}")
            ...
            with dot_right_column:
                ...
                with Y1:
                    st.session_state.select_dot_result=st.selectbox(
                        ...
                        #index=('dot_manager.csv', 'dot_manager_2.csv','dot_manager_3.csv').index(st.session_state.select_dot_note),
                        key='dot_key',  # Tab 1 でのみ使用
                        on_change=save_dot_selected_file()
                    )
                    #print("st.session_state.select_dot_result:{st.session_state.select_dot_result}")
                    #print('st.session_state.dot_key:{st.session_state.dot_key}')
                    #save_dot_selected_file()
                    
                    #print(f"[af]dot_note:{st.session_state.select_dot_note}")
                    #print(f"[af]dot_key:{st.session_state.dot_key}")
                    #print(f"[af]dot_result:{st.session_state.select_dot_result}")
                    # CSVファイル名
                    data_folder = os.path.join(os.getenv('APPDATA'), 'PeriOz2')
            ...
            with dot_left_column:
                ...
                dot_original_word = st.text_input("元の単語", key="dot_input1")
                ...
                with dot_second_left_column:
                    if st.button("セット登録",use_container_width=True,key="dot_set_reg_widget"):
                        if dot_original_word :
                            #identifier = str(uuid.uuid4())
                            dot_replacement_word = dot_original_word.replace(".","[dot]")
                            patterns = [
                ...
                with third_right_column_dot:
                    ...
                    if st.button("取り消し",use_container_width=True,key="dot_undo_widget"):
                        ...
                        if dot_previous_df is not None:
    ...
    elif st.session_state.current_tab == 'SRT/VTT→Excel':#☆makeExcel
        # print(f"st.session_state.ja_split:{st.session_state.ja_split}")
        # print(f"st.session_state.ja_split:{st.session_state.ja_split_key}")
        with st.container():
            #load_multi()
            ...
            option = st.radio("★ 選択してください:", ('日本語と英語', '英語のみ', '日本語のみ'),horizontal=True,on_change=load_multi)
            ...
            if option == '日本語と英語':
                ...
                with G1:
                    english_file = st.file_uploader("★ 英語のSRT/VTTファイルをアップロードしてください", type=['srt', 'vtt'], key=f'file_uploader_tab4_1_{st.session_state.uploader_tab4_1_key}')
                ...
                with G2:
                    japanese_file = st.file_uploader("★ 日本語のSRT/VTTファイルをアップロードしてください", type=['srt', 'vtt'], key=f'file_uploader_tab4_2_{st.session_state.uploader_tab4_2_key}')
                ...
                english_file = st.file_uploader("★ 英語のSRT/VTTファイルをアップロードしてください", type=['srt', 'vtt'], key=f'file_uploader_tab4_3_{st.session_state.uploader_tab4_3_key}')
                ...
                japanese_file = st.file_uploader("★ 日本語のSRT/VTTファイルをアップロードしてください", type=['srt', 'vtt'], key=f'file_uploader_tab4_4_{st.session_state.uploader_tab4_4_key}')
            ...
            with H3:
                if st.button("クリア", key='clear_tab4_widget',use_container_width=True):
                    if option=='日本語と英語':
                        moz.clear_session_state(tab='SRT/VTT→Excel',option="日本語と英語")
                    if option=='英語のみ':
                        moz.clear_session_state(tab='SRT/VTT→Excel',option='英語のみ')
                    if option=='日本語のみ':
                        moz.clear_session_state(tab='SRT/VTT→Excel',option='日本語のみ')
            ...
            if option == '日本語と英語':
                ...
                if do_exe_both:
                    if st.session_state.english_path_both and st.session_state.japanese_path_both:
                        ...
                        error_holder_column2.error("日本語ファイルをアップロードしてください。")
                    elif st.session_state.japanese_path_both:
                        error_holder_column1.error("英語のファイルをアップロードしてください。")
                    else:
                        error_holder_column2.error("日本語ファイルをアップロードしてください。")
                        error_holder_column1.error("英語のファイルをアップロードしてください。")
                ...
                if do_exe_en:
                    if st.session_state.english_path_only:
                        ...
                        error_holder_only_en.error("ファイルをアップロードしてください。")
                ...
                if do_exe_ja:
                    if st.session_state.japanese_path_only:
                        ...
                        error_holder_only_ja.error("ファイルをアップロードしてください。")
        ...
        with st.container():
            ...
            st.markdown("<div class='tab_title'>Youtube字幕の再編</div>", unsafe_allow_html=True)
